...
async function generateThemes(apiKey, type, language = "ru", previousThemes = [], contentMode = "vanilla") {
  ...
  const prompt = `Придумай 6 тем для ${typeName} нейро-арт челленджа.

ТРЕБОВАНИЯ:
- Каждая тема — ОБЯЗАТЕЛЬНО 2 предложения: первое описывает сцену, второе добавляет детали атмосферы
- Разнообразие: атмосфера, сюрреализм, эмоции, эстетика, мистика, простая красота
- Эмодзи и HTML теги (<b>жирный</b>, <i>курсив</i>) приветствуются
- ${modeDesc}${previousThemesNote}

ПРИМЕРЫ ТЕМ ИЗ 2 ПРЕДЛОЖЕНИЙ:
- "🌆 Уютная рамен-лавка поздней ночью, пар поднимается над тарелками. Тёплый свет бумажных фонариков отражается в мокром асфальте за окном."
...
- "🖤 Ведьма варит зелье в лесной хижине, зелёный дым клубится над котлом. Банки с ингредиентами на полках, чёрный кот внимательно наблюдает из угла."
- "✨ Пушистый кот свернулся калачиком на подоконнике старого дома. За окном идёт тихий дождь, внутри тепло и пахнет свежей выпечкой."

ЗАПРЕЩЕНО: имена персонажей (Гарри Поттер, Наруто), формат "X как Y", 6 похожих тем`;
