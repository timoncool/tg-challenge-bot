...
// Собирает AI конфиг из env переменных:
//   AI_PROVIDER = "gemini" | "openai"  (default: "gemini")
//   AI_API_URL  = custom endpoint      (default: auto по провайдеру)
//   AI_API_KEY  = ключ                 (fallback: GEMINI_API_KEY)
//   AI_MODEL    = модель               (default: "gemini-flash-lite-latest")
...
ВАРИАНТЫ ТЕМ (выбирай РАЗНЫЕ для каждой темы, НЕ все сразу):
• Персонаж в необычных условиях
• Мэшап стилей (киберпанк + гжель, викторианский sci-fi)
• Акцент на текстуре/материале (неон, вода, мех, золото, ржавчина)
• Ситуация/момент (только проснулась, сбежавшая невеста)
• Просто типаж или место (балерина, на сеновале, в музее)
