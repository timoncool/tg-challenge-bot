// ============================================
// MR. CHALLENGER BOT - Multi-Community Version
// Стильный куратор творческих соревнований
...
const ru = {
  ...
  // Опрос. Нейтрально и приглашающе.
  pollQuestion: (type) => {
    return `Время выбора. Какую тему возьмем в работу?`;
  },

  // Анонс. Стильно, мотивирующе.
...
Тема выбрана (${voteCount} голосов).
...
Токсичность и 🌚 здесь не приветствуются.
...
Достойно. Моё уважение.`;
  ...
  winnerAnnouncementFull: (username, score, type, topicFull) => {
    return `🏆 ЛУЧШАЯ РАБОТА

Автор: ${username}.
Оценка сообщества: ${score} ✨
...
Отличное исполнение.`;
  ...
  leaderboardTitle: (type) => {
    ...
    return `📜 Рейтинг лучших авторов (${labels[type]})`;
  },

  helpMessage: (schedule) => {
    const fmt = formatSchedule(schedule);
    return `Приветствую. Я Mr. Challenger.
...
Удачи в челленджах.`;
  ...
  getRandomReaction: () => {
    const reactions = ru.submissionReactions;
...
ВАЖНО: НЕ используй markdown, таблицы, заголовки или другое форматирование. Только простой текст.
</format>
...
<task>
Придумай 6 тем для ЕЖЕМЕСЯЧНОГО челленджа. Это ЭПИЧЕСКИЕ темы — масштабные, зрелищные, достойные портфолио!
</task>

<requirements>
- Каждая тема состоит из ДВУХ частей через разделитель " | ":
  1. ЗАГОЛОВОК (1 полное предложение) — полностью описывает идею, сразу понятно о чём тема
  2. ОПИСАНИЕ (4-5 предложений) — детали, персонажи, атмосфера, стиль, что изобразить
- ЗАГОЛОВОК должен быть ПОЛНЫМ ПРЕДЛОЖЕНИЕМ — читатель сразу понимает всю идею
- Темы должны быть ЭПИЧЕСКИМИ, КОНКРЕТНЫМИ и РАЗНООБРАЗНЫМИ:
  * Культовые персонажи в неожиданных стилях (киберпанк, Ренессанс, советский плакат, аниме)
  * Масштабные сцены из поп-культуры переосмысленные по-новому
  * Эпические "что если" сценарии с конкретными персонажами
  * Мэшапы эпох и стилей (средневековье + sci-fi, викторианская эпоха + аниме)
  * Грандиозные финальные битвы, кульминации, легендарные моменты
- ОБЯЗАТЕЛЬНО используй конкретных персонажей из: Marvel, DC, аниме, Disney/Pixar, игр, культовых фильмов
- НЕ ВСЕ темы должны быть кроссоверами! Разнообразь: переосмысления стиля, альтернативные версии, "что если"
- НЕ НУЖНА философия, метафизика, абстрактные концепции — нужны КРУТЫЕ ВИЗУАЛЬНЫЕ ИДЕИ
- Это вызов мастерству: сложные композиции, много деталей, wow-эффект
- Избегай: политики, религии, насилия, эротики
- Язык: русский
${previousThemesNote}
</requirements>
...
Тони Старк строит свой первый костюм в пещере — но в стиле советского конструктивизма 1920-х | Плакатная эстетика с красными и чёрными цветами. Железный Человек как символ индустриализации. Шестерёнки, заклёпки, пропагандистские надписи на броне. Пещера как завод будущего. Arc-реактор в форме звезды.
Все покемоны первого поколения собрались на групповое фото как выпускной класс | 151 покемон на ступеньках школы. Пикачу в центре с табличкой года. Чаризард пытается влезть в кадр сзади. Слоупок опоздал и бежит. Мьюту стоит в стороне как загадочный отличник. Ностальгия и детализация каждого персонажа.
Финальная битва Наруто и Саске переосмыслена в стиле японской гравюры укиё-э | Классические волны Хокусая, но это чакра. Традиционные японские мотивы с Расенганом и Чидори. Два силуэта на фоне горы Фудзи. Журавли и демоны-лисы в облаках. Музейное качество, золотая фольга, тушь.
</examples_for_format_only>`,
  };
...
Роковая женщина ждёт жертву в дымном баре 40-х годов | Классический нуар во всей красе. Сигаретный дым, кроваво-красная помада, взгляд, от которого бегут мурашки. Мужчины думают, что охотятся на неё — но охотник здесь она.
Доктор Лектер готовит изысканный ужин для особенного гостя | Элегантность высокой кухни и леденящая угроза в каждой детали. Идеально сервированный стол, классическая музыка, бокал кьянти. Вопрос лишь в том, что именно на тарелке.
Древний вампир создаёт профиль на сайте знакомств | Как выглядит анкета бессмертного существа ночи в Tinder? Интересы: готика, лунный свет, долгие прогулки по кладбищам. Ищу: O-отрицательную.
</examples_for_format_only>`,
...
Элегантная убийца на светском приёме скрывает кровь на перчатках | Женщина в вечернем платье среди гостей. Никто не замечает красные пятна на белых перчатках. Хрустальная люстра, шампанское, идеальная улыбка. Смерть среди роскоши.
    ...
    monthly: `<role>
Ты — креативный директор сообщества нейро-художников. Аудитория 16+ любит мрачную эстетику, нуар, готику и провокационные визуальные концепции.
</role>

<task>
Придумай 6 тем для ЕЖЕМЕСЯЧНОГО челленджа. ЭПИЧЕСКИЕ темы с тёмной взрослой эстетикой!
</task>

<requirements>
- Каждая тема состоит из ДВУХ частей через разделитель " | ":
  1. ЗАГОЛОВОК (1 полное предложение) — полностью описывает идею, сразу понятно о чём тема
  2. ОПИСАНИЕ (4-5 предложений) — детали, персонажи, атмосфера, что изобразить
- ЗАГОЛОВОК должен быть ПОЛНЫМ ПРЕДЛОЖЕНИЕМ — читатель сразу понимает всю идею
- Темы должны быть ЭПИЧЕСКИМИ, МРАЧНЫМИ и РАЗНООБРАЗНЫМИ:
  * Злодеи и антигерои как главные персонажи (Джокер, Веном, Дракула, леди Димитреску)
  * Культовые персонажи в мрачных стилях (готика, нуар, хоррор, дарк-фэнтези)
  * Тёмные "что если" сценарии — злодеи победили, герои пали, мир изменился
  * Эстетика роковых женщин, опасных мужчин, красивого зла
  * Атмосферные сцены: дождливые улицы, готические замки, неоновые подворотни
- ОБЯЗАТЕЛЬНО используй конкретных персонажей из: хорроров, нуар-фильмов, тёмного аниме, игр (Resident Evil, Dark Souls, Bloodborne)
- НЕ ВСЕ темы должны быть кроссоверами! Добавь: переосмысления стиля, тёмные версии персонажей, атмосферные сцены
- НЕ НУЖНА абстрактная философия и метафизика — нужны КРУТЫЕ МРАЧНЫЕ ВИЗУАЛЬНЫЕ ИДЕИ
- Это вызов мастерству: сложные композиции, много деталей, wow-эффект
...
<examples_for_format_only>
НЕ КОПИРУЙ! Создай ОРИГИНАЛЬНЫЕ концепции с другими персонажами и идеями.

Леди Димитреску устраивает бал для всех культовых вампиров кинематографа | Дракула Белы Лугоши танцует вальс с Акашей из "Королевы проклятых". Селена из Underworld у барной стойки с бокалом крови. Лестат играет на рояле. Готический замок, хрустальные люстры, гости в нарядах разных эпох. Элегантность бессмертия.
Весь Готэм в стиле картин Эдварда Хоппера — одиночество и неон | Бэтмен как силуэт в окне полуночной забегаловки. Женщина-кошка одна за барной стойкой. Джокер смотрит на город с крыши. Пустые улицы, резкие тени, атмосфера нуара и меланхолии. Каждый кадр — картина об изоляции.
Герои Dark Souls наконец нашли костёр, который не погаснет — и построили вокруг него город | Архитектура надежды посреди мрачного мира. Избранный мертвец стал королём. Жители — бывшие боссы и NPC, нашедшие покой. Тёплый свет костра против вечной тьмы за стенами. Что если история закончилась хорошо?
</examples_for_format_only>`,
  };
...
<task>
Придумай 6 тем для ЕЖЕМЕСЯЧНОГО челленджа. ЭПИЧЕСКИЕ эротические концепции — масштабные, зрелищные, достойные портфолио!
</task>
...
- ЗАГОЛОВОК должен быть ПОЛНЫМ ПРЕДЛОЖЕНИЕМ — читатель сразу понимает идею
- Темы должны быть ЭПИЧЕСКИМИ, КОНКРЕТНЫМИ и РАЗНООБРАЗНЫМИ:
  * Эротические версии культовых персонажей (аниме-вайфу, игровые героини, киноперсонажи)
  * Масштабные фантазии с детализацией (гаремы, оргии богов, вечеринки суккубов)
  * Конкретные фетиши в эпическом исполнении (latex ball, BDSM-замок, униформа-фестиваль)
  * Эротические переосмысления известных сцен и сеттингов
  * Мифология и фэнтези с откровенным содержанием
- ОБЯЗАТЕЛЬНО используй конкретику: персонажи, сеттинги, детали костюмов и поз
- НЕ ВСЕ темы должны быть про групповые сцены! Добавь: портреты, пары, соло
- НЕ НУЖНА философия — нужны ГОРЯЧИЕ ВИЗУАЛЬНЫЕ ИДЕИ с wow-эффектом
- НЕЛЬЗЯ: политика, религия, жестокость
- Язык: русский
${previousThemesNote}
</requirements>
...
<examples_for_format_only>
НЕ ПОВТОРЯЙ примеры! Создай УНИКАЛЬНЫЕ концепции — другие персонажи, фантазии, сеттинги.

Тифа Локхарт и Аэрис устроили совместную фотосессию для журнала в стиле 90-х | Эстетика глянцевых журналов эпохи PlayStation. Обе героини в откровенных версиях своих костюмов. Тифа в кожаных шортах и топе, Аэрис в прозрачном платье. Ретро-освещение, неоновые вывески Мидгара на фоне. Ностальгия встречает эротику.
Ведьма из Resident Evil Village принимает ванну в своём готическом замке | Леди Димитреску в огромной мраморной ванне, окружённая свечами и лепестками роз. Её дочери-вампирши прислуживают с полотенцами и вином. Пар, золотые краны, готическая роскошь. Размер и элегантность хозяйки замка во всей красе.
Все суккубы и инкубы демонического мира собрались на ежегодный бал соблазнения | Масштабная сцена в адском дворце. Демоны и демонессы в откровенных нарядах танцуют, флиртуют, соблазняют. Рога, хвосты, крылья, латекс и кожа. Огненное освещение, троны из костей, чаши с вином. Элегантный разврат высшего общества преисподней.
</examples_for_format_only>`,
  };
